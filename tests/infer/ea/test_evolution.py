...
def test_normal_normal():
    ...
    mutation_fns = {"loc": lambda x: dist.Normal(x, x.new_tensor(0.05)).sample(),
                    "scale": lambda x: dist.Normal(x, x.new_tensor(0.05)).sample()}

    loss = Parallelized_ELBO(model, guide, num_particles=10, num_chains=population_size, max_iarange_nesting=2)
    evol = Evolution(loss, mutation_fns, population_size=population_size, selection_size=15)
